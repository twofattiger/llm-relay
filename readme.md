...
├── .dev.vars.example     # 本地 secret 模板(复制为 .dev.vars 使用)
├── .gitignore
└── README.md             # 本文档
```

代码见 [`src/worker.js`](./src/worker.js),配置见 [`wrangler.toml`](./wrangler.toml)。本文档只讲原理、准备、部署与排错,不重复贴代码。
...
- **secret** 用 `npx wrangler secret put` 写入;**var** 写在 `wrangler.toml` 的 `[vars]`。
- 本地开发:复制 `.dev.vars.example` 为 `.dev.vars` 并填值(`.dev.vars` 已在 `.gitignore` 中)。
...
cp .dev.vars.example .dev.vars    # 本地开发用,填入真实值
```
...
```bash
npx wrangler dev    # http://localhost:8787,读取 .dev.vars
```
