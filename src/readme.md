...
llm-relay/
├── src/
│   ├── worker.js         # Worker 主代码(路由 + 鉴权 + 透传 + 协议转换 + 面板 + RelayStore DO)
...
Worker 校验对外 key 时,**同时接受两种头**:

- `Authorization: Bearer <key>` —— OpenAI SDK / 多数 OpenAI 兼容客户端用这个。
- `x-api-key: <key>` —— **Anthropic SDK、Claude Code 用这个**(它们不发 `Authorization`)。
...
**对外 key 有两类(校验顺序:master → isolate 缓存 → DO):**

1. **`MY_API_KEY`(master/兜底)** —— secret 注入,**始终有效、不依赖 DO**;DO 没配/故障也不会把自己锁在外面。
2. **动态 key** —— 在 `/admin` 面板增删,支持**命名 + 有效期 + 可用模型范围**(`allowed_models`,§6.9),存在公用 Durable Object `RelayStore`(SQLite)的 `api_keys` 表里。面板可复制 key,或复制其专属示例页链接 `/user/key/<key>` 发给使用者(详见 §6.7、§6.8)。

> 校验逻辑(`authClient`):先和 `MY_API_KEY` 恒定时间比对(master 视为不限模型);不中再查 **isolate 内存缓存**(命中即放行);未命中才问 DO,命中且**未禁用、未过期**则放行并写入缓存(默认 60s TTL)。鉴权同时取回该 key 的 `allowed_models`,随后按它校验请求的 model(§6.9)。
>
> **为什么用 DO 而非 KV**:DO(SQLite)强一致,吊销/计数即时、可 SQL 查询,且把 key、登录防爆破、将来的配额计数整合进**一个公用 DO**。代价是 DO 是单实例,故在热路径加 isolate 缓存避免每请求都打 DO(换来:吊销/改有效期最多滞后 ~TTL 生效,类似 KV)。**配额/请求次数(`limits`)将来用 DO 的 `usage_counters` 表原子自增**——这正是当初选 DO 的主要理由。
...
```
   ...
   POST /v1/chat/...      │  2. 对外 key 鉴权 (master 或 DO 动态 key;走 isolate 缓存)│
...
| 变量名 | 类型 | 说明 | 示例 |
|---|---|---|---|
| `MY_API_KEY` | **secret** | 对外的 **master/兜底 key**,始终有效、不依赖 DO;客户端在 `Authorization: Bearer` **或** `x-api-key` 里带它 | `sk-proxy-xxxxx` |
| `RELAY_DO` | **DO 绑定** | 公用存储 `RelayStore`(SQLite):动态 key + 登录防爆破 + 预留计数。在 `wrangler.toml` 配,**部署时迁移自动创建,无需手动建命名空间** | — |
...
| `ADMIN_PASSWORD` | **secret**(可选) | 配了才启用 `/admin` 面板(用量统计 + 动态 API Key 管理) | `xxxxxxxx` |
| `SUPPORT_LLMS` | var(可选) | 已上架模型清单,**JSON 字符串数组**(按客户端写法含前缀)。用于 key 的 `allowed_models` 全部/指定、示例下拉、`/v1/models`。空/不配=不作"全部/指定"约束(详见 §6.9) | `'''["@cf/x","deepseek/v4-flash"]'''` |
...
- `RELAY_DO` 是 **Durable Object 绑定**(非 secret/var),已在 `src/wrangler.toml` 配好(类 `RelayStore` + 迁移)。不绑定也能跑:仅 `MY_API_KEY` 有效、面板 Key 管理不可用、登录防爆破跳过。
...
> **Durable Object 免手动建**:`wrangler.toml` 里的 `RelayStore` DO 及其迁移(v1 建 + v2 rename)会在 **deploy 时自动应用**,无需像 KV 那样先 `kv namespace create`。首次部署会自动建好 SQLite 存储。
>
...
```bash
npx wrangler secret put MY_API_KEY      # master/兜底 key(始终有效;更多 key 在面板动态签发)
npx wrangler secret put CF_API_TOKEN    # Cloudflare API token(AI Gateway Run + Workers AI Read)
# 可选:npx wrangler secret put ADMIN_PASSWORD   # 启用 /admin 面板(统计 + 动态 Key 管理)
...
### 6.7 动态 API Key 管理(/admin 面板)

配了 `ADMIN_PASSWORD` 并绑定了 `RELAY_DO` 后,登录 `/admin` 面板即可在"**API Key 管理**"区动态签发对外 key,无需改 secret 或重新部署。

- **新建**:填名称(可选)、选有效期、选**可用模型范围** → 生成 `sk-relay-...` 形式的 key。
  - **有效期**:快捷下拉(永不 / 7 / 30 / 90 / 365 天)或选 **`自定义…`** 用日期时间控件挑具体到期时刻。
  - **可用模型**:下拉三选一——`不限` / `全部(列表内)` / `指定…`;选"指定"时从 `SUPPORT_LLMS` 勾选若干。默认 **不限**。语义见 §6.9。
- **禁用 / 启用 / 删除**:即时写 DO(强一致),但因热路径有 isolate 缓存,**改动最多约 60s 后全网生效**(可调小 `worker.js` 的 `KEY_CACHE_TTL_MS`)。
- **有效期**:`expires_at` 存毫秒时间戳;到期自动失效(DO 校验时按 `expires_at` 判,缓存也不会续命)。自定义时间须晚于当前,否则后端返回"到期时间需晚于当前时间"。
- **每行两个复制按钮**(Key 列不再显示明文,避免过长):
  - **`复制 apiKey`** —— 复制 key 本身。
  - **`复制示例`** —— 复制该 key 的**专属示例页链接** `https://<你的域名>/user/key/<key>`,发给使用者即可。
- **`支持模型` 列**:`不限` / `全部` / `部分`(点击弹窗列出该 key 的允许模型)。
- 存储:DO `RelayStore` 的 `api_keys` 表,字段 `name / created_at / expires_at / disabled / allowed_models / limits`。`allowed_models` **已启用**(§6.9);`limits` 仍为配额预留位。

```
key 优先级:MY_API_KEY(master,始终有效) > isolate 缓存 > DO 动态 key(未禁用且未过期)
```

> 这些 key 与 `MY_API_KEY` 用法完全一样:`Authorization: Bearer <key>` 或 `x-api-key: <key>`,前缀路由规则不变。
>
> **公用 DO 还承载**:登录防爆破(`login_guard` 表)、以及预留的**用量计数**(`usage_counters` 表)。配额/请求次数功能实现时,会在 DO 内对 key 原子自增计数(强一致),不必再引入别的存储。

### 6.8 接入「示例」与用户示例页(`/user/key/<key>`)

面板和用户页共用一份服务端「示例」生成器(`buildExamples(origin, key)`),内容含 **cURL(四条路由)+ Cursor + Claude Code + OpenCode** 的接入方式,并把示例里的 `<MY_API_KEY>` 占位**直接换成真实 key**:

- **面板「示例」区**(原"请求示例"):经鉴权接口 `GET /admin/api/examples` 拉取,用占位符 **`<MY_API_KEY>`** 填充(不暴露真实 master key,自行替换成你的 key)。
- **用户示例页 `GET /user/key/<key>`**:把链接发给使用者,页面**只展示**他自己的 key + 用该 key 填好的全部示例(带复制按钮),无任何管理功能。
  - key 自证、**无需登录**;但会先校验 key **存在 / 未禁用 / 未过期**(同请求鉴权那套),无效或为空直接返回错误页。
  - `<key>` 为空或失效 → 403 错误页"Key 无效或已失效"。
- **示例上方有模型下拉**,切换即把示例里的 model 换成所选。下拉来源按该 key 的 `allowed_models`:`-1`/`0` → 显示 `SUPPORT_LLMS` 全部;指定列表 → 显示该 key 的允许模型(§6.9)。

> 典型用法:管理员在面板给某人建一把带有效期、限定模型的 key → 点该行「复制示例」拿到 `…/user/key/<key>` → 发给对方。对方打开就能看到填好自己 key 的 cURL / Cursor / Claude Code / OpenCode 接入方法,照抄即可。

### 6.9 模型白名单 `SUPPORT_LLMS` 与按 key 限制 `allowed_models`

**`SUPPORT_LLMS`**(`wrangler.toml` 的 `[vars]`,**JSON 字符串数组**)是一份"已上架模型"清单,按**客户端写法**(含前缀)填,例:

```toml
SUPPORT_LLMS = '''["@cf/meta/llama-3.2-3b-instruct", "deepseek/deepseek-v4-flash", "@/anthropic/claude-sonnet-4-5"]'''
```

它有三处用途:① 作为 `allowed_models = 0` 的"全部"来源;② 面板/用户页模型下拉与新建时"指定"的候选;③ `GET /v1/models` 列表。

**模型校验已从"全局白名单"改为"按当前 key 的 `allowed_models`"**(校验顺序:鉴权拿到 key 的 `allowed_models` → 判模型)。`allowed_models` 三态:

| 值 | 含义 | 校验 |
|---|---|---|
| `-1`(或空 / 历史无值 / master) | **不限** | 不比对,**可用任意模型**(含不在 `SUPPORT_LLMS` 的) |
| `0` | **全部** | 传入 model 必须在 `SUPPORT_LLMS` 中(**列表后期扩充自动生效**,无需改 key) |
| `a,b,c`(`;` 或 `,` 分隔) | **指定列表** | 传入 model 必须在该列表中 |

> 为何 `0` 不直接把模型写进字段:这样后期 `SUPPORT_LLMS` 新增模型时,`0` 的 key 自动覆盖,免去逐个更新。
> 不通过返回 `403 Model '<model>' is not allowed for this key`。`-1` 是放给"自己用、想随便填模型"的场景。

**`GET /v1/models`**(OpenAI 格式,需有效 key)返回 `SUPPORT_LLMS`:

```bash
curl https://<你的worker>/v1/models -H "Authorization: Bearer <你的key>"
# { "object":"list", "data":[ {"id":"deepseek/deepseek-v4-flash","object":"model","created":...,"owned_by":"system"}, ... ] }
```

> 等价于 Ollama 的 `/api/tags`。OpenAI SDK 启动常会调它做校验,正好可用。注意它返回完整 `SUPPORT_LLMS`,不按 key 的 `allowed_models` 过滤。
...
| 面板"API Key 管理"提示 **DO 未绑定(RELAY_DO)** | 没配 DO 绑定/迁移 | 确认 `wrangler.toml` 有 `RelayStore` 绑定与迁移,重新 `deploy` |
| 新建的动态 key 立刻用却 401 / 删了还能用一会儿 | 热路径 isolate 缓存(默认 60s) | 等约 1 分钟;要即时用 `MY_API_KEY`,或调小 `KEY_CACHE_TTL_MS` |
| deploy 报 DO migration / class 相关错 | 迁移链与已部署状态不一致 | 见 `wrangler.toml` 注释:全新可只留一条 `new_sqlite_classes=["RelayStore"]`;已部署过 LoginGuard 则保留 v1+v2 rename |
| 打开 `/user/key/<key>` 显示"Key 无效或已失效" | key 为空/不存在/已禁用/已过期 | 在面板确认该 key 状态;刚禁用的因缓存可能仍有效约 60s |
| `403 Model '<model>' is not allowed for this key` | 该 model 不在此 key 的 `allowed_models` 范围 | 改用允许的 model;或在面板把该 key 调成 `不限`/`全部`/加进指定列表;`0` 的 key 需该 model 在 `SUPPORT_LLMS` 中(§6.9) |

---

## 8. 安全与限制

- **凭证隔离**:对外用 `MY_API_KEY`(master)或面板签发的动态 key(DO);CF 账号 token(`CF_API_TOKEN`)、BYOK key 都在服务端/CF 托管。`MY_API_KEY` 轮换改 secret 即可;动态 key 在面板随时增删/禁用。
- **动态 key 明文存 DO**:为支持面板查看复制,key 明文存于 DO 的 `api_keys` 表。其暴露面与 BYOK key、`CF_API_TOKEN` 同级——都需 CF 账号访问权才能读;要求更高可改存哈希(只在创建时显示一次)。
- **吊销有 ~TTL 窗口**:热路径 isolate 缓存换来低延迟,代价是禁用/删除动态 key 后最多滞后 `KEY_CACHE_TTL_MS`(默认 60s)生效;需要立即失效的高危场景改用 master key 或临时调小 TTL。
- **DO 单实例**:`RelayStore` 是全球单实例,低/中流量无感;真到高并发时它是鉴权瓶颈,但 isolate 缓存已挡掉绝大多数请求。配额计数也在此 DO 内原子完成(强一致)。
...
- **限流/配额**:当前未启用按 key 限额(`api_keys.limits` 是预留位)。实现时在公用 DO `RelayStore` 的 `usage_counters` 表里**原子自增**计数(强一致),按 key 维度判额;**不要用 KV 计数**(最终一致 + 写限额会丢数)。网关级限流也可在 AI Gateway 侧配。
...
1. **服务端模型白名单/别名表**:已有 `SUPPORT_LLMS`(§6.9)做白名单;可再扩成"各模型走哪条路、属于哪个格式族"的映射表,对调用方屏蔽前缀细节、自动选路由与格式入口。
2. **多 BYOK key 切换**:按调用方身份注入 `cf-aig-byok-alias`,实现 dev/prod key 隔离。
3. **`/v1/models` 合成端点**:✅ 已实现(返回 `SUPPORT_LLMS`,OpenAI 格式,§6.9)。如需真实上游目录,可再合成(Workers AI 用 `GET /accounts/{id}/ai/models/search`)。
4. **按 key 计量与配额**:基础设施已就位——公用 DO `RelayStore` 已建 `usage_counters` 表、`api_keys.limits` 字段也预留。补上"每请求按 key 在 DO 内原子自增 + 判额"即可,无需新存储。
