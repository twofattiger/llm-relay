...
2. **动态 key** —— 在 `/admin` 面板增删,支持**命名 + 有效期**,明文存储、面板可查看复制。存在公用 Durable Object `RelayStore`(SQLite)的 `api_keys` 表里(详见 §6.7)。
...
- **新建**:填名称(可选)、选有效期(永不 / 7 / 30 / 90 / 365 天)→ 生成 `sk-relay-...` 形式的 key,明文展示、可复制。
- **禁用 / 启用 / 删除**:即时写 DO(强一致),但因热路径有 isolate 缓存,**改动最多约 60s 后全网生效**(可调小 `worker.js` 的 `KEY_CACHE_TTL_MS`)。
- **有效期**:到期自动失效(DO 校验时按 `expires_at` 判,缓存也不会续命)。
