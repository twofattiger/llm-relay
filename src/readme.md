...
2. **动态 key** —— 在 `/admin` 面板增删,支持**命名 + 有效期**(快捷下拉或自定义日期),存在公用 Durable Object `RelayStore`(SQLite)的 `api_keys` 表里。面板可复制 key,或复制其专属示例页链接 `/user/key/<key>` 发给使用者(详见 §6.7、§6.8)。

> 校验逻辑(`authClient`):先和 `MY_API_KEY` 恒定时间比对;不中再查 **isolate 内存缓存**(命中即放行);未命中才问 DO,命中且**未禁用、未过期**则放行并写入缓存(默认 60s TTL)。
>
...
- **新建**:填名称(可选)、选有效期——快捷下拉(永不 / 7 / 30 / 90 / 365 天)或选 **`自定义…`** 用日期时间控件挑一个具体到期时刻 → 生成 `sk-relay-...` 形式的 key。
...
- 存储:DO `RelayStore` 的 `api_keys` 表,字段 `name / created_at / expires_at / disabled / allowed_models / limits`,后两者为预留扩展位(允许的模型、配额)。
...
> 典型用法:管理员在面板给某人建一把带有效期的 key → 点该行「复制示例」拿到 `…/user/key/<key>` → 发给对方。对方打开就能看到填好自己 key 的 cURL / Cursor / Claude Code / OpenCode 接入方法,照抄即可。
